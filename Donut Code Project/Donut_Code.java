...
public class Donut_Code 
{
    ...
    public static double K1 = 70;//screenwidth*K2*3/(8*(R1+R2));
    ...
    public static void initiateTorus(int c) 
    {
        ...
        for (double theta = 0; theta < 2*Math.PI; theta += .07)
        {
            for (double phi = 0; phi < 2*Math.PI; phi += .02)
            {
                ...
                double x = xo + crosssection_x*(Math.cos(c*B)*Math.cos(phi) + Math.sin(c*A)*Math.sin(c*B)*Math.sin(phi)) - crosssection_y*Math.cos(c*A)*Math.sin(c*B);
                double y = yo + crosssection_x*(Math.sin(c*B)*Math.cos(phi) - Math.sin(c*A)*Math.cos(c*B)*Math.sin(phi)) + crosssection_y*Math.cos(c*A)*Math.cos(c*B);
                double z = zo + Math.cos(c*A)*crosssection_x*Math.sin(phi) + crosssection_y*Math.sin(c*A);
